...
(function (root, factory) {
  ...
  Lightbox.prototype.start = function ($link) {
    ...
    function addToAlbum($link, isVideo) {
      self.album.push({
    ...
    if (dataLightboxValue) {
      ...
      for (var i = 0; i < $links.length; i = ++i) {
        var fileType = $links.eq(i).attr("href").split(".").pop().toLowerCase();
        var isVideo =
          fileType === "mp4" || fileType === "webm" || fileType === "ogg";
        addToAlbum($($links[i]), isVideo);
        if ($links[i] === $link[0]) {
          imageNumber = i;
        }
      }
  ...
  Lightbox.prototype.changeImage = function (imageNumber) {
    ...
    var videoTag = $(
      '<video controls class="lb-video" autoplay ><source src="' +
    ...
    if (self.album[imageNumber].isVideo) {
      if ($(".lb-video")) $(".lb-video").remove();
      videoTag.insertAfter($image);

      var $videoElement = this.$lightbox.find(".lb-video");
      $videoElement.on("loadedmetadata", function () {
        self.sizeContainer(
          $videoElement.outerWidth(),
          $videoElement.outerHeight()
        );
      });
      if ($(".lb-image")) $(".lb-image")[0].hidden = true;
    } else {
      $(".lb-image")[0].hidden = false;
      if (videoTag) $(".lb-video").remove();
